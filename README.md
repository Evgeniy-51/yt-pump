...
> Пользователь сам отвечает за соблюдение авторского права и правил YouTube и других площадок. YtPump не обходит возрастные ограничения, региональные блокировки и требования входа в аккаунт.
...
- **Автоматический анализ ссылки.** После вставки URL программа сама запрашивает у yt-dlp список доступных разрешений, звуковых дорожек и субтитров.
...
- **Прокси** HTTP / HTTPS / SOCKS5 с несколькими сохранёнными профилями и встроенной проверкой.
...
- Если прокси включён, но профиль не заполнен, окно настроек откроется автоматически.
...
`bin/`, `obj/`, `dist/`, `data/`, `*.zip`, `*.pdb`, а также бинарники `tools/*.exe` и `tools/*.dll` — они скачиваются `fetch-tools.ps1`. В репозитории остаются только `tools/README.txt` и тексты лицензий.
